...
MODEL_ROOT = "/home/han/study/projects/agent-motion-prediction/models/lanegcn/"
...
if "save_dir" not in config:
    config["save_dir"] = os.path.join(
        MODEL_ROOT, "results", model_name
    )
    
if not os.path.isabs(config["save_dir"]):
    config["save_dir"] = os.path.join(MODEL_ROOT, "results", config["save_dir"])
...
"""Dataset"""
# Raw Dataset
# Note: these are paths relative to DATA_ROOT which is set as "L5KIT_DATA_FOLDER" environment variable
config["train_data_loader"] = "scenes/splited/train_zarr_splited_valid/train_zarr_0.zarr"
config["val_data_loader"] = "scenes/splited/validate_zarr_splited_valid/validate_zarr_0.zarr"
config["test_data_loader"] = "scenes/splited/test_zarr_splited/test_zarr_0.zarr"

# Preprocessed Dataset
config["preprocess"] = True # whether use preprocess or not
config["preprocess_train"] = os.path.join(PREPROCESSED_ROOT, "combined_train", "train_crs_dist6_combined.p")
config["preprocess_val"] = os.path.join(PREPROCESSED_ROOT, "combined_val", "val_crs_dist6_combined.p")
config["preprocess_test"] = os.path.join(PREPROCESSED_ROOT, "preprocessed_test", "test_test.p")
...
#         agent_from_world = gpu(data["agent_from_world"])
#         # transform prediction to world coordinates
#         for i in range(len(out["reg"])):
#             num_agents, num_mods, T_preds = out["reg"][i].size()[:3]
#             out_add_column_one = torch.cat((out["reg"][i], torch.ones(num_agents, num_mods, T_preds, 1).cuda()), -1)
#             world_from_agent = torch.inverse(agent_from_world[i]).float()
#             out["reg"][i] = torch.transpose(torch.matmul(world_from_agent, torch.transpose(out_add_column_one, 2, 3)), 2, 3)[:, :, :, :2]
...
#         self.meta = Linear(n_map + 4, n_map, norm=norm, ng=ng)
        self.meta = Linear(n_map, n_map, norm=norm, ng=ng)  # modified by Tang Han
        att = []
        for i in range(2):
...
#             ),
#             1,
#         )  # Commented out by Tang Han, I don't extract turn, control or intersect info
...
#     assert has_preds.all()  # commented out by Tang Han
...
def get_model():
    ...
    return config, LyftDataset, collate_fn, net, loss, post_process, opt
