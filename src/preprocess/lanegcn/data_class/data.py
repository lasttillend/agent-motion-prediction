...
class LyftDataset(Dataset):
    ...
    def __init__(self, config, mode="train"):
        ...
        if "preprocess" in config and config["preprocess"]:
            ...
            print("zarr_dataset path:", dataset_path)
            ...
            self.scene_track_ids = self.find_scene_track_ids()
    ...
    def find_scene_track_ids(self):
        scene_track_ids = dict()  # scene_id -> ndarray of unique_agents
        for scene_id in range(len(self.scenes)):
            ...
            scene_track_ids[scene_id] = track_ids
            
        return scene_track_ids

    def filter_agents(self, data):
        """
        A valid agent must satisfy the following three conditions:
        ...
        for i in range(len(steps) - 1):
            if 19 not in steps[i]:
                agents_to_remove.append(i)
                continue
            history_frames = steps[i][steps[i] < 20]
            future_frames = steps[i][steps[i] >= 20]
            if len(history_frames) < MIN_FRAME_HISTORY or len(future_frames) < MIN_FRAME_FUTURE:
                agents_to_remove.append(i)
    ...
    def read_lyft_data(self, idx):
        ...
        for track_id in self.scene_track_ids[scene_id]:
...
if __name__ == "__main__":
    cfg = {
        ...
        'train_data_loader': 'scenes/sample.zarr',
  
        'num_scales': 6,
        'pred_range': [-100.0, 100.0, -100.0, 100.0],
    }
    dataset = LyftDataset(cfg)
    data = dataset[0]
    # print(data.keys())
